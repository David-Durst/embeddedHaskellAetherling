...
#pragma HLS INTERFACE ap_hs port=hw_output
 ...
 // alias the arguments
 hls::stream<AxiPackedStencil<uint8_t, 2, 1> > &_hw_input_stencil_stream = hw_input;
 hls::stream<AxiPackedStencil<uint8_t, 2, 1> > &_hw_output_stencil_stream = hw_output;

 // dispatch_stream(_hw_input_stencil_stream, 2, 2, 2, 200, 1, 1, 1, 1, "hw_output", 1, 0, 200, 0, 1);
 hls::stream<AxiPackedStencil<uint8_t, 2, 1> > _hw_input_stencil_stream_to_hw_output;
...
#pragma HLS RESOURCE variable=_hw_input_stencil_stream_to_hw_output core=FIFO_SRL
 ...
 for (int _dim_0 = 0; _dim_0 <= 198; _dim_0 += 2)
 {
#pragma HLS PIPELINE
  PackedStencil<uint8_t, 2, 1> _tmp_stencil = _hw_input_stencil_stream.read();
  if (_dim_0 >= 0 && _dim_0 <= 198 && _dim_1 >= 0 && _dim_1 <= 0)
  {
...
#pragma HLS PIPELINE II=1
   Stencil<uint8_t, 2, 1> _hw_input_stencil;
...
#pragma HLS ARRAY_PARTITION variable=_hw_output_stencil.value complete dim=0
   ...
   if (_hw_output_x___scan_dim_0 == 99 && _hw_output_y___scan_dim_1 == 0) {
