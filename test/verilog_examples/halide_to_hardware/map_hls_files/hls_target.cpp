...
//void hls_target(
    ...
    //int hw_output[1])
//{
//#pragma HLS INTERFACE axis port=hw_input
//#pragma HLS INTERFACE axis port=hw_output
  //hw_output[0] = hw_input[0];
//}
...
//#pragma HLS INTERFACE ap_fifo port=hw_input depth=10
//#pragma HLS INTERFACE ap_fifo port=hw_output depth=10
#pragma HLS INTERFACE axis port=hw_input depth=1
#pragma HLS INTERFACE axis port=hw_output depth=1
  hw_output.write(hw_input.read());
  //for (int i = 0; i < 200; i++) {
    //Stencil<uint8_t, 1, 1> iv = hw_input.read();
    //Stencil<uint8_t, 1, 1> _hw_input_stencil;
    //_hw_input_stencil(0, 0) = iv(0, 0) + 1;
    //hw_output.write(_hw_input_stencil);
  //}
}
...
//#pragma HLS INTERFACE ap_hs port=hw_output
 ...
 //hls::stream<AxiPackedStencil<uint8_t, 1, 1> > &_hw_output_stencil_stream = hw_output;
   ...
   //Stencil<uint8_t, 1, 1> _hw_input_stencil;
   //_hw_input_stencil(0, 0) = 0;

   //AxiPackedStencil<uint8_t, 1, 1> _hw_output_stencil_packed = _hw_input_stencil;
