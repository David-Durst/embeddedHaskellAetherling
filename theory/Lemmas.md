...
When looking at a type such as:
...
1. Values in the same column - these types arrive on the same clock. Therefore, writing them to the same bank will cause a conflict on the input port
1. Values in the same contiguous block of length n where n is the SSeq dimension. On the output, these types will be emitted at the same time
     1. Indexes are generated by flattening the types and counting from 0 to total length - 1.

## Converting Port Conflict Rules To Indexes
note: `s` is `SSeq` index, `t` is `TSeq` index
1. Values in the same column -
    1. SSeq (TSeq) - `s`
    1. TSeq (SSeq) - `s`
1. Values in the same contiguous block of n
    1. SSeq (TSeq) - `flat_idx % sseq_dim`
    1. TSeq (SSeq) - `s`

## Converting Between Indexes
1. TSeq (SSeq) `s` = SSeq (TSeq) `flat_idx % sseq_dim` = TSeq (SSeq) `flat_idx % sseq_dim`
1. TSeq (SSeq) `t` = SSeq (TSeq) `flat_idx / sseq_dim` = TSeq (SSeq) `flat_idx / sseq_dim`
...
1. Let `flat_idx` be the index of each element if sequences are flattened
1. Let `s` be the current index in `SSeq`
1. Let `t` be the current index in `TSeq`
1. If inner num elements and outer num elements are coprime, 
    1. `bank = s`
    1. `addr = t`
1. If inner num elements and outer num elements are not coprime,
