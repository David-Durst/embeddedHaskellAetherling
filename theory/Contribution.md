...
1. Darkroom
    ...
    1. Analogous to clock calculus - p. 4-5 - finding the minimal buffering between stages so can have synchronous circuits.
...
1. Lift
    ...
    1. Analogous to space-time IR - p. 76 of **Lift: A Functional Data-Parallel IR for High-Performance GPU Code Generation** - implement map/reduce primitives that are fully parallel and sequential in OpenCL
        1. p. 78-9 - Use dependent types on operator's inputs and outputs to size memories
