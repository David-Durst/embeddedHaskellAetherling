...
get_area' e@(Counter_tsN no io ni _ out_type _ _) = do
  -- add 1 one for storage and 1 for counter
  return $ (ni + 2) * size_t out_type
get_area' e@(Counter_tnN nis iis _ out_type _ _) = do
  -- add n for counters and 1 for counter value
  return $ (length nis + 1) * size_t out_type
