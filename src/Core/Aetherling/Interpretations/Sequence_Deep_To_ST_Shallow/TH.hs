module Aetherling.Interpretations.Sequence_Deep_To_ST_Shallow.TH where
...
data Liftable n = Liftable deriving Lift
addComponent = [| addC |]
absTH = [| absC |]
map_sTH :: (KnownNat n) => Liftable n -> Q Exp -> Q Exp
map_sTH n f = [| mapC (Proxy :: Proxy n) $f |]
{-
