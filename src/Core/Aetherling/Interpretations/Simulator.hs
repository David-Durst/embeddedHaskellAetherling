...
instance Sequence_Language Simulation_Env where
  ...
  seq_tuple_to_seqC (Simulation_Env (Seq input_seq_tuples)) = do
    let input_tuples = fmap (\(Seq x) -> V.head x) input_seq_tuples
  ...
  seq_to_seq_tupleC (Simulation_Env (Seq input_seqs)) =
    return $ Seq $ fmap (\(Seq x) -> (list_to_seq (Proxy @1) [Seq_Tuple x])) input_seqs
